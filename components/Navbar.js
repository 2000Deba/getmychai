  ...
  return (
    <nav className='bg-blue-950 text-white flex justify-between items-center px-4 md:h-16 flex-col md:flex-row'>
      ...
      <div className='relative flex justify-center items-center  md:block gap-4' ref={dropdownRef}>
        ...
        {session ? (<button className="text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 cursor-pointer" onClick={() => signOut()}>Logout</button>) : (
          <Link href="/login">
            <button className="text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 cursor-pointer">Login</button></Link>)}
      </div>
    </nav>
  )
}
